  ...
  <section class="wrap zones-wrap" aria-labelledby="zones-title">
    ...
    <div class="controls">
      <div
          class="chips"
          role="tablist"
          :aria-label="$t('map.aria.filterByCat')"
      >
        <button
              ...
              <ul class="info-list" role="list">
                <li>
                  ...
                  <svg class="i i--stroke" viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M12 21c0 0-7-7.2-7-12a7 7 0 1 1 14 0c0 4.8-7 12-7 12Z"/>
...
export default defineComponent({
  ...
  setup() {
    ...
    const places = HUARIQUES.map(h => {
      ...
      return { id: slug(h.name), name: h.name, cat }
    })
    const META = {}
    HUARIQUES.forEach(h => {
    ...
    function tagsByCat(code, price, rating, near){
      const tgs = []
      // Etiqueta de categoría traducida
    ...
    const chipOptions = computed(() => {
      return [{ cat:'all', label: t('map.filters.all') }].concat(
          allCatsInData.map(code => ({
            cat: code,
            label: t(`map.filters.${code}`)
          }))
      )
    })
    ...
    onMounted(()=>{
      ...
      places.forEach(p=>{
        ...
        mk.bindPopup(
            `<img src="${logoUrl(m.logo)}" alt="${p.name}" class="popup-logo">`,
            { autoPan:true, closeButton:true, maxWidth:220, className:'ps-popup' }
        )
