...
const form = reactive({
  name: '', categoryId: '', address: '',
  phone: '', imgUrl: '', description: ''
});
...
<template>
  <form class="card" @submit="onSubmit" style="max-width:720px;margin:0 auto">
    ...
    <input v-model.trim="form.categoryId" placeholder="ej. anticuchos, ceviche" required />
    ...
    <div style="display:flex;gap:12px;justify-content:flex-end;margin-top:14px">
      <button type="submit" class="ps-btn" :disabled="submitting">{{ submitText }}</button>
    </div>
  </form>
</template>

<style scoped>
.card{ background:#fff; border-radius:18px; padding:18px; box-shadow: var(--shadow-1,0 6px 18px rgba(0,0,0,.06)); }
...
.ps-btn{ padding:10px 16px; border:none; border-radius:18px; background:var(--orange,#e3891b); color:#fff; cursor:pointer; }
.ps-btn[disabled]{ opacity:.6; cursor:default; }
</style>
